...
export async function generateMetadata(): Promise<Metadata> {
  const page = await getAboutPage("pl");
  return {
    title: page?.metaTitle ?? "About Carelabs | Poland Electrical Safety Experts",
    description:
      page?.metaDescription ??
      "Learn about Carelabs — our mission, values, and the team dedicated to electrical safety testing and PN-HD 60364 compliance across Poland.",
    alternates: {
      canonical: "https://carelabz.com/pl/about-us/",
      languages: {
        "en-PL": "https://carelabz.com/pl/about-us/",
        "x-default": "https://carelabz.com/pl/about-us/",
      },
    },
    openGraph: {
      title: page?.metaTitle ?? "About Carelabs — Power System Consultants Poland",
      description:
        page?.metaDescription ?? "Carelabs is a leading electrical safety engineering firm in Poland.",
      url: "https://carelabz.com/pl/about-us/",
      siteName: "Carelabs",
      type: "website",
    },
    twitter: {
...
export default async function PLAboutPage() {
  const page = await getAboutPage("pl");

  const headline = page?.heroHeadline ?? "Who We Are";
  const subtext =
    page?.heroSubtext ??
    "Carelabs is a trusted partner for electrical safety testing, calibration, inspection, and certification services across Poland.";
  ...
  const jsonLd = buildJsonLd([
    ...
    getWebPageSchema(
      "https://carelabz.com/pl/about-us/",
      page?.metaTitle ?? "About Carelabs — Power System Consultants Poland",
      page?.metaDescription ?? "Carelabs is a leading electrical safety engineering firm in Poland.",
      "en-PL"
    ),
    getBreadcrumbSchema([
      { name: "Home", url: "https://carelabz.com/pl/" },
      { name: "About", url: "https://carelabz.com/pl/about-us/" },
    ]),
  ]);
        ...
        <section className="bg-[#0050B3] py-20 px-4">
          <div className="mx-auto max-w-4xl text-center">
            <h2 className="text-3xl md:text-4xl font-bold text-white mb-4">
              {page?.ctaBannerHeading ?? "Partner with Carelabs Poland"}
            </h2>
            {page?.ctaBannerSubtext ? (
              <p className="text-white/70 text-lg mb-8">{page.ctaBannerSubtext}</p>
            ) : (
              <p className="text-white/70 text-lg mb-8">
                Ready to elevate your safety standards? Talk to our Poland experts today.
              </p>
            )}
            <Link
              href={page?.ctaBannerPrimaryHref ?? "/pl/contact-us/"}
