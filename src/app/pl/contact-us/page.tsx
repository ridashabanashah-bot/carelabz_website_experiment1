...
export async function generateMetadata(): Promise<Metadata> {
  const page = await getContactPage("pl");
  return {
    title: page?.metaTitle ?? "Contact Carelabs Poland | Get in Touch",
    description:
      page?.metaDescription ??
      "Contact the Carelabs team for electrical safety testing, arc flash studies, and PN-HD 60364 compliance services across Poland.",
    alternates: {
      canonical: "https://carelabz.com/pl/contact-us/",
      languages: {
        "en-PL": "https://carelabz.com/pl/contact-us/",
        "x-default": "https://carelabz.com/pl/contact-us/",
      },
    },
    openGraph: {
      title: page?.metaTitle ?? "Contact Carelabs — Get a Free Consultation Poland",
      description:
        page?.metaDescription ?? "Get in touch for a free consultation on electrical safety services in Poland.",
      url: "https://carelabz.com/pl/contact-us/",
      siteName: "Carelabs",
      type: "website",
    },
    twitter: {
...
export default async function PLContactPage() {
  const headersList = headers();
  const iso2 = getCountryFromHeaders(headersList, "PL");
  // On the Poland contact page the banner must always say Poland
  // regardless of the visitor's IP — the services ARE in Poland.
  const countryName = "Poland";

  const [page, services] = await Promise.all([
    getContactPage("pl"),
    getServicesByRegion("pl"),
  ]);
  ...
  const subtext =
    page?.heroSubtext ??
    "Have a question or ready to start a project? Our Poland team is here to help.";
  ...
  const jsonLd = buildJsonLd([
    ...
    getWebPageSchema(
      "https://carelabz.com/pl/contact-us/",
      page?.metaTitle ?? "Contact Carelabs — Get a Free Consultation Poland",
      page?.metaDescription ?? "Get in touch for a free consultation on electrical safety services in Poland.",
      "en-PL"
    ),
    getBreadcrumbSchema([
      { name: "Home", url: "https://carelabz.com/pl/" },
      { name: "Contact", url: "https://carelabz.com/pl/contact-us/" },
    ]),
  ]);
                ...
                {page?.mapEmbedUrl && (
                  <div className="rounded-xl overflow-hidden border border-gray-200 mt-4">
                    <iframe
                      ...
                      title="Carelabs Poland Office Location"
                    />
                  </div>
                )}
              </div>
        ...
        <section className="bg-[#0050B3] py-20 px-4">
          <div className="mx-auto max-w-4xl text-center">
            ...
            <p className="text-white/70 text-lg mb-8">
              Our Poland team is available to respond to emergency compliance and
