...
export async function generateMetadata(): Promise<Metadata> {
  const page = await getAboutPage("hu");
  return {
    title: page?.metaTitle ?? "About Carelabs | Hungary Electrical Safety Experts",
    description:
      page?.metaDescription ??
      "Learn about Carelabs — our mission, values, and the team dedicated to electrical safety testing and MSZ HD 60364 compliance across Hungary.",
    alternates: {
      canonical: "https://carelabz.com/hu/about-us/",
      languages: {
        "en-HU": "https://carelabz.com/hu/about-us/",
        "x-default": "https://carelabz.com/hu/about-us/",
      },
    },
    openGraph: {
      title: page?.metaTitle ?? "About Carelabs — Power System Consultants Hungary",
      description:
        page?.metaDescription ?? "Carelabs is a leading electrical safety engineering firm in Hungary.",
      url: "https://carelabz.com/hu/about-us/",
      siteName: "Carelabs",
      type: "website",
    },
    twitter: {
...
export default async function HUAboutPage() {
  const page = await getAboutPage("hu");

  const headline = page?.heroHeadline ?? "Who We Are";
  const subtext =
    page?.heroSubtext ??
    "Carelabs is a trusted partner for electrical safety testing, calibration, inspection, and certification services across Hungary.";
  ...
  const jsonLd = buildJsonLd([
    ...
    getWebPageSchema(
      "https://carelabz.com/hu/about-us/",
      page?.metaTitle ?? "About Carelabs — Power System Consultants Hungary",
      page?.metaDescription ?? "Carelabs is a leading electrical safety engineering firm in Hungary.",
      "en-HU"
    ),
    getBreadcrumbSchema([
      { name: "Home", url: "https://carelabz.com/hu/" },
      { name: "About", url: "https://carelabz.com/hu/about-us/" },
    ]),
  ]);
        ...
        <section className="bg-[#0050B3] py-20 px-4">
          <div className="mx-auto max-w-4xl text-center">
            <h2 className="text-3xl md:text-4xl font-bold text-white mb-4">
              {page?.ctaBannerHeading ?? "Partner with Carelabs Hungary"}
            </h2>
            {page?.ctaBannerSubtext ? (
              <p className="text-white/70 text-lg mb-8">{page.ctaBannerSubtext}</p>
            ) : (
              <p className="text-white/70 text-lg mb-8">
                Ready to elevate your safety standards? Talk to our Hungary experts today.
              </p>
            )}
            <Link
              href={page?.ctaBannerPrimaryHref ?? "/hu/contact-us/"}
