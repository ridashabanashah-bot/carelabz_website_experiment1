...
export async function generateMetadata(): Promise<Metadata> {
  const page = await getContactPage("hu");
  return {
    title: page?.metaTitle ?? "Contact Carelabs Hungary | Get in Touch",
    description:
      page?.metaDescription ??
      "Contact the Carelabs team for electrical safety testing, arc flash studies, and MSZ HD 60364 compliance services across Hungary.",
    alternates: {
      canonical: "https://carelabz.com/hu/contact-us/",
      languages: {
        "en-HU": "https://carelabz.com/hu/contact-us/",
        "x-default": "https://carelabz.com/hu/contact-us/",
      },
    },
    openGraph: {
      title: page?.metaTitle ?? "Contact Carelabs — Get a Free Consultation Hungary",
      description:
        page?.metaDescription ?? "Get in touch for a free consultation on electrical safety services in Hungary.",
      url: "https://carelabz.com/hu/contact-us/",
      siteName: "Carelabs",
      type: "website",
    },
    twitter: {
...
export default async function HUContactPage() {
  const headersList = headers();
  const iso2 = getCountryFromHeaders(headersList, "HU");
  // On the Hungary contact page the banner must always say Hungary
  // regardless of the visitor's IP — the services ARE in Hungary.
  const countryName = "Hungary";

  const [page, services] = await Promise.all([
    getContactPage("hu"),
    getServicesByRegion("hu"),
  ]);
  ...
  const subtext =
    page?.heroSubtext ??
    "Have a question or ready to start a project? Our Hungary team is here to help.";
  ...
  const jsonLd = buildJsonLd([
    ...
    getWebPageSchema(
      "https://carelabz.com/hu/contact-us/",
      page?.metaTitle ?? "Contact Carelabs — Get a Free Consultation Hungary",
      page?.metaDescription ?? "Get in touch for a free consultation on electrical safety services in Hungary.",
      "en-HU"
    ),
    getBreadcrumbSchema([
      { name: "Home", url: "https://carelabz.com/hu/" },
      { name: "Contact", url: "https://carelabz.com/hu/contact-us/" },
    ]),
  ]);
                ...
                {page?.mapEmbedUrl && (
                  <div className="rounded-xl overflow-hidden border border-gray-200 mt-4">
                    <iframe
                      ...
                      title="Carelabs Hungary Office Location"
                    />
                  </div>
                )}
              </div>
        ...
        <section className="bg-[#0050B3] py-20 px-4">
          <div className="mx-auto max-w-4xl text-center">
            ...
            <p className="text-white/70 text-lg mb-8">
              Our Hungary team is available to respond to emergency compliance and
