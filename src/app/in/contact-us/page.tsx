...
export async function generateMetadata(): Promise<Metadata> {
  const page = await getContactPage("in");
  return {
    title: page?.metaTitle ?? "Contact Carelabs India | Get in Touch",
    description:
      page?.metaDescription ??
      "Contact the Carelabs team for electrical safety testing, arc flash studies, and CEA Regulations 2010 compliance services across India.",
    alternates: {
      canonical: "https://carelabz.com/in/contact-us/",
      languages: {
        "en-IN": "https://carelabz.com/in/contact-us/",
        "x-default": "https://carelabz.com/in/contact-us/",
      },
    },
    openGraph: {
      title: page?.metaTitle ?? "Contact Carelabs — Get a Free Consultation India",
      description:
        page?.metaDescription ?? "Get in touch for a free consultation on electrical safety services in India.",
      url: "https://carelabz.com/in/contact-us/",
      siteName: "Carelabs",
      type: "website",
    },
    twitter: {
...
export default async function INContactPage() {
  const headersList = headers();
  const iso2 = getCountryFromHeaders(headersList, "IN");
  // On the India contact page the banner must always say India
  // regardless of the visitor's IP — the services ARE in India.
  const countryName = "India";

  const [page, services] = await Promise.all([
    getContactPage("in"),
    getServicesByRegion("in"),
  ]);
  ...
  const subtext =
    page?.heroSubtext ??
    "Have a question or ready to start a project? Our Indian team is here to help.";

  const jsonLd = buildJsonLd([
    getRegionOrganizationSchema({ cc: "in", countryName: "India", countryIso2: "IN", phone: config.phone, email: config.email, addressLocality: config.address }),
    getWebPageSchema(
      "https://carelabz.com/in/contact-us/",
      page?.metaTitle ?? "Contact Carelabs — Get a Free Consultation India",
      page?.metaDescription ?? "Get in touch for a free consultation on electrical safety services in India.",
      "en-IN"
    ),
    getBreadcrumbSchema([
      { name: "Home", url: "https://carelabz.com/in/" },
      { name: "Contact", url: "https://carelabz.com/in/contact-us/" },
    ]),
  ]);
                ...
                {page?.mapEmbedUrl && (
                  <div className="rounded-xl overflow-hidden border border-gray-200 mt-4">
                    <iframe
                      ...
                      title="Carelabs India Office Location"
                    />
                  </div>
                )}
              </div>
        ...
        <section className="bg-[#0050B3] py-20 px-4">
          <div className="mx-auto max-w-4xl text-center">
            ...
            <p className="text-white/70 text-lg mb-8">
              Our Indian team is available to respond to emergency compliance and
