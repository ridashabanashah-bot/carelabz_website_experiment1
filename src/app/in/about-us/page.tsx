...
export async function generateMetadata(): Promise<Metadata> {
  const page = await getAboutPage("in");
  return {
    title: page?.metaTitle ?? "About Carelabs | Indian Electrical Safety Experts",
    description:
      page?.metaDescription ??
      "Learn about Carelabs — our mission, values, and the team dedicated to electrical safety testing and CEA Regulations 2010 compliance across India.",
    alternates: {
      canonical: "https://carelabz.com/in/about-us/",
      languages: {
        "en-IN": "https://carelabz.com/in/about-us/",
        "x-default": "https://carelabz.com/in/about-us/",
      },
    },
    openGraph: {
      title: page?.metaTitle ?? "About Carelabs — Power System Consultants India",
      description:
        page?.metaDescription ?? "Carelabs is a leading electrical safety engineering firm in India.",
      url: "https://carelabz.com/in/about-us/",
      siteName: "Carelabs",
      type: "website",
    },
    twitter: {
...
export default async function INAboutPage() {
  const page = await getAboutPage("in");

  const headline = page?.heroHeadline ?? "Who We Are";
  const subtext =
    page?.heroSubtext ??
    "Carelabs is a trusted partner for electrical safety testing, calibration, inspection, and certification services across India.";
  ...
  const jsonLd = buildJsonLd([
    ...
    getWebPageSchema(
      "https://carelabz.com/in/about-us/",
      page?.metaTitle ?? "About Carelabs — Power System Consultants India",
      page?.metaDescription ?? "Carelabs is a leading electrical safety engineering firm in India.",
      "en-IN"
    ),
    getBreadcrumbSchema([
      { name: "Home", url: "https://carelabz.com/in/" },
      { name: "About", url: "https://carelabz.com/in/about-us/" },
    ]),
  ]);
        ...
        <section className="bg-[#0050B3] py-20 px-4">
          <div className="mx-auto max-w-4xl text-center">
            <h2 className="text-3xl md:text-4xl font-bold text-white mb-4">
              {page?.ctaBannerHeading ?? "Partner with Carelabs India"}
            </h2>
            {page?.ctaBannerSubtext ? (
              <p className="text-white/70 text-lg mb-8">{page.ctaBannerSubtext}</p>
            ) : (
              <p className="text-white/70 text-lg mb-8">
                Ready to elevate your safety standards? Talk to our Indian experts today.
              </p>
            )}
            <Link
              href={page?.ctaBannerPrimaryHref ?? "/in/contact-us/"}
