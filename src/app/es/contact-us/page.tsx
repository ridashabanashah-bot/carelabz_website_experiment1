...
const config = COUNTRY_CONFIGS["es"];
...
export async function generateMetadata(): Promise<Metadata> {
  const page = await getContactPage("es");
  return {
    title: page?.metaTitle ?? "Contact Carelabs Spain | Get in Touch",
    description:
      page?.metaDescription ??
      "Contact the Carelabs team for electrical safety testing, arc flash studies, and REBT compliance services across Spain.",
    alternates: {
      canonical: "https://carelabz.com/es/contact-us/",
      languages: {
        "en-ES": "https://carelabz.com/es/contact-us/",
        "x-default": "https://carelabz.com/es/contact-us/",
      },
    },
    openGraph: {
      title: page?.metaTitle ?? "Contact Carelabs — Get a Free Consultation Spain",
      description:
        page?.metaDescription ?? "Get in touch for a free consultation on electrical safety services in Spain.",
      url: "https://carelabz.com/es/contact-us/",
      siteName: "Carelabs",
      type: "website",
    },
    twitter: {
...
export default async function ESContactPage() {
  ...
  const iso2 = getCountryFromHeaders(headersList, "ES");
  // On the Spain contact page the banner must always say Spain
  // regardless of the visitor's IP — the services ARE in Spain.
  const countryName = "Spain";

  const [page, services] = await Promise.all([
    getContactPage("es"),
    getServicesByRegion("es"),
  ]);
  ...
  const subtext =
    page?.heroSubtext ??
    "Have a question or ready to start a project? Our Spain team is here to help.";

  const jsonLd = buildJsonLd([
    getRegionOrganizationSchema({ cc: "es", countryName: "Spain", countryIso2: "ES", phone: config.phone, email: config.email, addressLocality: config.address }),
    getWebPageSchema(
      "https://carelabz.com/es/contact-us/",
      page?.metaTitle ?? "Contact Carelabs — Get a Free Consultation Spain",
      page?.metaDescription ?? "Get in touch for a free consultation on electrical safety services in Spain.",
      "en-ES"
    ),
    getBreadcrumbSchema([
      { name: "Home", url: "https://carelabz.com/es/" },
      { name: "Contact", url: "https://carelabz.com/es/contact-us/" },
    ]),
  ]);
                ...
                {page?.mapEmbedUrl && (
                  <div className="rounded-xl overflow-hidden border border-gray-200 mt-4">
                    <iframe
                      ...
                      title="Carelabs Spain Office Location"
                    />
                  </div>
                )}
              </div>
        ...
        <section className="bg-[#0050B3] py-20 px-4">
          <div className="mx-auto max-w-4xl text-center">
            ...
            <p className="text-white/70 text-lg mb-8">
              Our Spain team is available to respond to emergency compliance and
