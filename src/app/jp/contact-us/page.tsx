...
const config = COUNTRY_CONFIGS["jp"];
...
export async function generateMetadata(): Promise<Metadata> {
  const page = await getContactPage("jp");
  return {
    title: page?.metaTitle ?? "Contact Carelabs Japan | Get in Touch",
    description:
      page?.metaDescription ??
      "Contact the Carelabs team for electrical safety testing, arc flash studies, and JEAC 8001 compliance services across Japan.",
    alternates: {
      canonical: "https://carelabz.com/jp/contact-us/",
      languages: {
        "en-JP": "https://carelabz.com/jp/contact-us/",
        "x-default": "https://carelabz.com/jp/contact-us/",
      },
    },
    openGraph: {
      title: page?.metaTitle ?? "Contact Carelabs — Get a Free Consultation Japan",
      description:
        page?.metaDescription ?? "Get in touch for a free consultation on electrical safety services in Japan.",
      url: "https://carelabz.com/jp/contact-us/",
      siteName: "Carelabs",
      type: "website",
    },
    twitter: {
...
export default async function JPContactPage() {
  const headersList = headers();
  const iso2 = getCountryFromHeaders(headersList, "JP");
  // On the Japan contact page the banner must always say Japan
  // regardless of the visitor's IP — the services ARE in Japan.
  const countryName = "Japan";

  const [page, services] = await Promise.all([
    getContactPage("jp"),
    getServicesByRegion("jp"),
  ]);
  ...
  const subtext =
    page?.heroSubtext ??
    "Have a question or ready to start a project? Our Japan team is here to help.";

  const jsonLd = buildJsonLd([
    getRegionOrganizationSchema({ cc: "jp", countryName: "Japan", countryIso2: "JP", phone: config.phone, email: config.email, addressLocality: config.address }),
    getWebPageSchema(
      "https://carelabz.com/jp/contact-us/",
      page?.metaTitle ?? "Contact Carelabs — Get a Free Consultation Japan",
      page?.metaDescription ?? "Get in touch for a free consultation on electrical safety services in Japan.",
      "en-JP"
    ),
    getBreadcrumbSchema([
      { name: "Home", url: "https://carelabz.com/jp/" },
      { name: "Contact", url: "https://carelabz.com/jp/contact-us/" },
    ]),
  ]);
                ...
                {page?.mapEmbedUrl && (
                  <div className="rounded-xl overflow-hidden border border-gray-200 mt-4">
                    <iframe
                      ...
                      title="Carelabs Japan Office Location"
                    />
                  </div>
                )}
              </div>
        ...
        <section className="bg-[#0050B3] py-20 px-4">
          <div className="mx-auto max-w-4xl text-center">
            ...
            <p className="text-white/70 text-lg mb-8">
              Our Japan team is available to respond to emergency compliance and
