...
export async function generateMetadata(): Promise<Metadata> {
  const page = await getAboutPage("jp");
  return {
    title: page?.metaTitle ?? "About Carelabs | Japan Electrical Safety Experts",
    description:
      page?.metaDescription ??
      "Learn about Carelabs — our mission, values, and the team dedicated to electrical safety testing and JEAC 8001 compliance across Japan.",
    alternates: {
      canonical: "https://carelabz.com/jp/about-us/",
      languages: {
        "en-JP": "https://carelabz.com/jp/about-us/",
        "x-default": "https://carelabz.com/jp/about-us/",
      },
    },
    openGraph: {
      title: page?.metaTitle ?? "About Carelabs — Power System Consultants Japan",
      description:
        page?.metaDescription ?? "Carelabs is a leading electrical safety engineering firm in Japan.",
      url: "https://carelabz.com/jp/about-us/",
      siteName: "Carelabs",
      type: "website",
    },
    twitter: {
...
export default async function JPAboutPage() {
  const page = await getAboutPage("jp");

  const headline = page?.heroHeadline ?? "Who We Are";
  const subtext =
    page?.heroSubtext ??
    "Carelabs is a trusted partner for electrical safety testing, calibration, inspection, and certification services across Japan.";
  ...
  const jsonLd = buildJsonLd([
    ...
    getWebPageSchema(
      "https://carelabz.com/jp/about-us/",
      page?.metaTitle ?? "About Carelabs — Power System Consultants Japan",
      page?.metaDescription ?? "Carelabs is a leading electrical safety engineering firm in Japan.",
      "en-JP"
    ),
    getBreadcrumbSchema([
      { name: "Home", url: "https://carelabz.com/jp/" },
      { name: "About", url: "https://carelabz.com/jp/about-us/" },
    ]),
  ]);
        ...
        <section className="bg-[#0050B3] py-20 px-4">
          <div className="mx-auto max-w-4xl text-center">
            <h2 className="text-3xl md:text-4xl font-bold text-white mb-4">
              {page?.ctaBannerHeading ?? "Partner with Carelabs Japan"}
            </h2>
            {page?.ctaBannerSubtext ? (
              <p className="text-white/70 text-lg mb-8">{page.ctaBannerSubtext}</p>
            ) : (
              <p className="text-white/70 text-lg mb-8">
                Ready to elevate your safety standards? Talk to our Japan experts today.
              </p>
            )}
            <Link
              href={page?.ctaBannerPrimaryHref ?? "/jp/contact-us/"}
