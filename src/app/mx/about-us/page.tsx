...
export async function generateMetadata(): Promise<Metadata> {
  const page = await getAboutPage("mx");
  return {
    title: page?.metaTitle ?? "About Carelabs | Mexico Electrical Safety Experts",
    description:
      page?.metaDescription ??
      "Learn about Carelabs — our mission, values, and the team dedicated to electrical safety testing and NOM-029-STPS-2011 compliance across Mexico.",
    alternates: {
      canonical: "https://carelabz.com/mx/about-us/",
      languages: {
        "en-MX": "https://carelabz.com/mx/about-us/",
        "x-default": "https://carelabz.com/mx/about-us/",
      },
    },
    openGraph: {
      title: page?.metaTitle ?? "About Carelabs — Power System Consultants Mexico",
      description:
        page?.metaDescription ?? "Carelabs is a leading electrical safety engineering firm in Mexico.",
      url: "https://carelabz.com/mx/about-us/",
      siteName: "Carelabs",
      type: "website",
    },
    twitter: {
...
export default async function MXAboutPage() {
  const page = await getAboutPage("mx");

  const headline = page?.heroHeadline ?? "Who We Are";
  const subtext =
    page?.heroSubtext ??
    "Carelabs is a trusted partner for electrical safety testing, calibration, inspection, and certification services across Mexico.";
  ...
  const jsonLd = buildJsonLd([
    ...
    getWebPageSchema(
      "https://carelabz.com/mx/about-us/",
      page?.metaTitle ?? "About Carelabs — Power System Consultants Mexico",
      page?.metaDescription ?? "Carelabs is a leading electrical safety engineering firm in Mexico.",
      "en-MX"
    ),
    getBreadcrumbSchema([
      { name: "Home", url: "https://carelabz.com/mx/" },
      { name: "About", url: "https://carelabz.com/mx/about-us/" },
    ]),
  ]);
        ...
        <section className="bg-[#0050B3] py-20 px-4">
          <div className="mx-auto max-w-4xl text-center">
            <h2 className="text-3xl md:text-4xl font-bold text-white mb-4">
              {page?.ctaBannerHeading ?? "Partner with Carelabs Mexico"}
            </h2>
            {page?.ctaBannerSubtext ? (
              <p className="text-white/70 text-lg mb-8">{page.ctaBannerSubtext}</p>
            ) : (
              <p className="text-white/70 text-lg mb-8">
                Ready to elevate your safety standards? Talk to our Mexico experts today.
              </p>
            )}
            <Link
              href={page?.ctaBannerPrimaryHref ?? "/mx/contact-us/"}
