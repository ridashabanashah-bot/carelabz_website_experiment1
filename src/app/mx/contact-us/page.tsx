...
export async function generateMetadata(): Promise<Metadata> {
  const page = await getContactPage("mx");
  return {
    title: page?.metaTitle ?? "Contact Carelabs Mexico | Get in Touch",
    description:
      page?.metaDescription ??
      "Contact the Carelabs team for electrical safety testing, arc flash studies, and NOM-029-STPS-2011 compliance services across Mexico.",
    alternates: {
      canonical: "https://carelabz.com/mx/contact-us/",
      languages: {
        "en-MX": "https://carelabz.com/mx/contact-us/",
        "x-default": "https://carelabz.com/mx/contact-us/",
      },
    },
    openGraph: {
      title: page?.metaTitle ?? "Contact Carelabs — Get a Free Consultation Mexico",
      description:
        page?.metaDescription ?? "Get in touch for a free consultation on electrical safety services in Mexico.",
      url: "https://carelabz.com/mx/contact-us/",
      siteName: "Carelabs",
      type: "website",
    },
    twitter: {
...
export default async function MXContactPage() {
  const headersList = headers();
  const iso2 = getCountryFromHeaders(headersList, "MX");
  // On the Mexico contact page the banner must always say Mexico
  // regardless of the visitor's IP — the services ARE in Mexico.
  const countryName = "Mexico";

  const [page, services] = await Promise.all([
    getContactPage("mx"),
    getServicesByRegion("mx"),
  ]);
  ...
  const subtext =
    page?.heroSubtext ??
    "Have a question or ready to start a project? Our Mexico team is here to help.";
  ...
  const jsonLd = buildJsonLd([
    ...
    getWebPageSchema(
      "https://carelabz.com/mx/contact-us/",
      page?.metaTitle ?? "Contact Carelabs — Get a Free Consultation Mexico",
      page?.metaDescription ?? "Get in touch for a free consultation on electrical safety services in Mexico.",
      "en-MX"
    ),
    getBreadcrumbSchema([
      { name: "Home", url: "https://carelabz.com/mx/" },
      { name: "Contact", url: "https://carelabz.com/mx/contact-us/" },
    ]),
  ]);
                ...
                {page?.mapEmbedUrl && (
                  <div className="rounded-xl overflow-hidden border border-gray-200 mt-4">
                    <iframe
                      ...
                      title="Carelabs Mexico Office Location"
                    />
                  </div>
                )}
              </div>
        ...
        <section className="bg-[#0050B3] py-20 px-4">
          <div className="mx-auto max-w-4xl text-center">
            ...
            <p className="text-white/70 text-lg mb-8">
              Our Mexico team is available to respond to emergency compliance and
