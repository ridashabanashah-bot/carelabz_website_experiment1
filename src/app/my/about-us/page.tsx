...
export async function generateMetadata(): Promise<Metadata> {
  const page = await getAboutPage("my");
  return {
    title: page?.metaTitle ?? "About Carelabs | Malaysian Electrical Safety Experts",
    description:
      page?.metaDescription ??
      "Learn about Carelabs — our mission, values, and the team dedicated to electrical safety testing and MS IEC 60364 compliance across Malaysia.",
    alternates: {
      canonical: "https://carelabz.com/my/about-us/",
      languages: {
        "en-MY": "https://carelabz.com/my/about-us/",
        "x-default": "https://carelabz.com/my/about-us/",
      },
    },
    openGraph: {
      title: page?.metaTitle ?? "About Carelabs — Power System Consultants Malaysia",
      description:
        page?.metaDescription ?? "Carelabs is a leading electrical safety engineering firm in Malaysia.",
      url: "https://carelabz.com/my/about-us/",
      siteName: "Carelabs",
      type: "website",
    },
    twitter: {
...
export default async function MYAboutPage() {
  const page = await getAboutPage("my");

  const headline = page?.heroHeadline ?? "Who We Are";
  const subtext =
    page?.heroSubtext ??
    "Carelabs is a trusted partner for electrical safety testing, calibration, inspection, and certification services across Malaysia.";
  ...
  const jsonLd = buildJsonLd([
    ...
    getWebPageSchema(
      "https://carelabz.com/my/about-us/",
      page?.metaTitle ?? "About Carelabs — Power System Consultants Malaysia",
      page?.metaDescription ?? "Carelabs is a leading electrical safety engineering firm in Malaysia.",
      "en-MY"
    ),
    getBreadcrumbSchema([
      { name: "Home", url: "https://carelabz.com/my/" },
      { name: "About", url: "https://carelabz.com/my/about-us/" },
    ]),
  ]);
        ...
        <section className="bg-[#0050B3] py-20 px-4">
          <div className="mx-auto max-w-4xl text-center">
            <h2 className="text-3xl md:text-4xl font-bold text-white mb-4">
              {page?.ctaBannerHeading ?? "Partner with Carelabs Malaysia"}
            </h2>
            {page?.ctaBannerSubtext ? (
              <p className="text-white/70 text-lg mb-8">{page.ctaBannerSubtext}</p>
            ) : (
              <p className="text-white/70 text-lg mb-8">
                Ready to elevate your safety standards? Talk to our Malaysian experts today.
              </p>
            )}
            <Link
              href={page?.ctaBannerPrimaryHref ?? "/my/contact-us/"}
