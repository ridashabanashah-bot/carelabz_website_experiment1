...
const config = COUNTRY_CONFIGS["ua"];
...
export async function generateMetadata(): Promise<Metadata> {
  const page = await getContactPage("ua");
  return {
    title: page?.metaTitle ?? "Contact Carelabs Ukraine | Get in Touch",
    description:
      page?.metaDescription ??
      "Contact the Carelabs team for electrical safety testing, arc flash studies, and PUE compliance services across Ukraine.",
    alternates: {
      canonical: "https://carelabz.com/ua/contact-us/",
      languages: {
        "en-UA": "https://carelabz.com/ua/contact-us/",
        "x-default": "https://carelabz.com/ua/contact-us/",
      },
    },
    openGraph: {
      title: page?.metaTitle ?? "Contact Carelabs — Get a Free Consultation Ukraine",
      description:
        page?.metaDescription ?? "Get in touch for a free consultation on electrical safety services in Ukraine.",
      url: "https://carelabz.com/ua/contact-us/",
      siteName: "Carelabs",
      type: "website",
    },
    twitter: {
...
export default async function UAContactPage() {
  const headersList = headers();
  const iso2 = getCountryFromHeaders(headersList, "UA");
  // On the Ukraine contact page the banner must always say Ukraine
  // regardless of the visitor's IP — the services ARE in Ukraine.
  const countryName = "Ukraine";

  const [page, services] = await Promise.all([
    getContactPage("ua"),
    getServicesByRegion("ua"),
  ]);
  ...
  const subtext =
    page?.heroSubtext ??
    "Have a question or ready to start a project? Our Ukraine team is here to help.";

  const jsonLd = buildJsonLd([
    getRegionOrganizationSchema({ cc: "ua", countryName: "Ukraine", countryIso2: "UA", phone: config.phone, email: config.email, addressLocality: config.address }),
    getWebPageSchema(
      "https://carelabz.com/ua/contact-us/",
      page?.metaTitle ?? "Contact Carelabs — Get a Free Consultation Ukraine",
      page?.metaDescription ?? "Get in touch for a free consultation on electrical safety services in Ukraine.",
      "en-UA"
    ),
    getBreadcrumbSchema([
      { name: "Home", url: "https://carelabz.com/ua/" },
      { name: "Contact", url: "https://carelabz.com/ua/contact-us/" },
    ]),
  ]);
                ...
                {page?.mapEmbedUrl && (
                  <div className="rounded-xl overflow-hidden border border-gray-200 mt-4">
                    <iframe
                      ...
                      title="Carelabs Ukraine Office Location"
                    />
                  </div>
                )}
              </div>
        ...
        <section className="bg-[#0050B3] py-20 px-4">
          <div className="mx-auto max-w-4xl text-center">
            ...
            <p className="text-white/70 text-lg mb-8">
              Our Ukraine team is available to respond to emergency compliance and
