...
export async function generateMetadata(): Promise<Metadata> {
  const page = await getAboutPage("ua");
  return {
    title: page?.metaTitle ?? "About Carelabs | Ukraine Electrical Safety Experts",
    description:
      page?.metaDescription ??
      "Learn about Carelabs — our mission, values, and the team dedicated to electrical safety testing and PUE compliance across Ukraine.",
    alternates: {
      canonical: "https://carelabz.com/ua/about-us/",
      languages: {
        "en-UA": "https://carelabz.com/ua/about-us/",
        "x-default": "https://carelabz.com/ua/about-us/",
      },
    },
    openGraph: {
      title: page?.metaTitle ?? "About Carelabs — Power System Consultants Ukraine",
      description:
        page?.metaDescription ?? "Carelabs is a leading electrical safety engineering firm in Ukraine.",
      url: "https://carelabz.com/ua/about-us/",
      siteName: "Carelabs",
      type: "website",
    },
    twitter: {
...
export default async function UAAboutPage() {
  const page = await getAboutPage("ua");

  const headline = page?.heroHeadline ?? "Who We Are";
  const subtext =
    page?.heroSubtext ??
    "Carelabs is a trusted partner for electrical safety testing, calibration, inspection, and certification services across Ukraine.";
  ...
  const jsonLd = buildJsonLd([
    ...
    getWebPageSchema(
      "https://carelabz.com/ua/about-us/",
      page?.metaTitle ?? "About Carelabs — Power System Consultants Ukraine",
      page?.metaDescription ?? "Carelabs is a leading electrical safety engineering firm in Ukraine.",
      "en-UA"
    ),
    getBreadcrumbSchema([
      { name: "Home", url: "https://carelabz.com/ua/" },
      { name: "About", url: "https://carelabz.com/ua/about-us/" },
    ]),
  ]);
        ...
        <section className="bg-[#0050B3] py-20 px-4">
          <div className="mx-auto max-w-4xl text-center">
            <h2 className="text-3xl md:text-4xl font-bold text-white mb-4">
              {page?.ctaBannerHeading ?? "Partner with Carelabs Ukraine"}
            </h2>
            {page?.ctaBannerSubtext ? (
              <p className="text-white/70 text-lg mb-8">{page.ctaBannerSubtext}</p>
            ) : (
              <p className="text-white/70 text-lg mb-8">
                Ready to elevate your safety standards? Talk to our Ukraine experts today.
              </p>
            )}
            <Link
              href={page?.ctaBannerPrimaryHref ?? "/ua/contact-us/"}
