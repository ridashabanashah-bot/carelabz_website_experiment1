...
export async function generateMetadata(): Promise<Metadata> {
  const page = await getContactPage("de");
  return {
    title: page?.metaTitle ?? "Contact Carelabs Germany | Get in Touch",
    description:
      page?.metaDescription ??
      "Contact the Carelabs team for electrical safety testing, arc flash studies, and DIN VDE 0100 compliance services across Germany.",
    alternates: {
      canonical: "https://carelabz.com/de/contact-us/",
      languages: {
        "en-DE": "https://carelabz.com/de/contact-us/",
        "x-default": "https://carelabz.com/de/contact-us/",
      },
    },
    openGraph: {
      title: page?.metaTitle ?? "Contact Carelabs — Get a Free Consultation Germany",
      description:
        page?.metaDescription ?? "Get in touch for a free consultation on electrical safety services in Germany.",
      url: "https://carelabz.com/de/contact-us/",
      siteName: "Carelabs",
      type: "website",
    },
    twitter: {
...
export default async function DEContactPage() {
  ...
  const iso2 = getCountryFromHeaders(headersList, "DE");
  // On the Germany contact page the banner must always say Germany
  // regardless of the visitor's IP — the services ARE in Germany.
  const countryName = "Germany";

  const [page, services] = await Promise.all([
    getContactPage("de"),
    getServicesByRegion("de"),
  ]);
  ...
  const subtext =
    page?.heroSubtext ??
    "Have a question or ready to start a project? Our Germany team is here to help.";
  ...
  const jsonLd = buildJsonLd([
    ...
    getWebPageSchema(
      "https://carelabz.com/de/contact-us/",
      page?.metaTitle ?? "Contact Carelabs — Get a Free Consultation Germany",
      page?.metaDescription ?? "Get in touch for a free consultation on electrical safety services in Germany.",
      "en-DE"
    ),
    getBreadcrumbSchema([
      { name: "Home", url: "https://carelabz.com/de/" },
      { name: "Contact", url: "https://carelabz.com/de/contact-us/" },
    ]),
  ]);
                ...
                {page?.mapEmbedUrl && (
                  <div className="rounded-xl overflow-hidden border border-gray-200 mt-4">
                    <iframe
                      ...
                      title="Carelabs Germany Office Location"
                    />
                  </div>
                )}
              </div>
        ...
        <section className="bg-[#0050B3] py-20 px-4">
          <div className="mx-auto max-w-4xl text-center">
            ...
            <p className="text-white/70 text-lg mb-8">
              Our Germany team is available to respond to emergency compliance and
