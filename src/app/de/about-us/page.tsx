...
export async function generateMetadata(): Promise<Metadata> {
  const page = await getAboutPage("de");
  return {
    title: page?.metaTitle ?? "About Carelabs | Germany Electrical Safety Experts",
    description:
      page?.metaDescription ??
      "Learn about Carelabs — our mission, values, and the team dedicated to electrical safety testing and DIN VDE 0100 compliance across Germany.",
    alternates: {
      canonical: "https://carelabz.com/de/about-us/",
      languages: {
        "en-DE": "https://carelabz.com/de/about-us/",
        "x-default": "https://carelabz.com/de/about-us/",
      },
    },
    openGraph: {
      title: page?.metaTitle ?? "About Carelabs — Power System Consultants Germany",
      description:
        page?.metaDescription ?? "Carelabs is a leading electrical safety engineering firm in Germany.",
      url: "https://carelabz.com/de/about-us/",
      siteName: "Carelabs",
      type: "website",
    },
    twitter: {
...
export default async function DEAboutPage() {
  const page = await getAboutPage("de");

  const headline = page?.heroHeadline ?? "Who We Are";
  const subtext =
    page?.heroSubtext ??
    "Carelabs is a trusted partner for electrical safety testing, calibration, inspection, and certification services across Germany.";
  ...
  const jsonLd = buildJsonLd([
    ...
    getWebPageSchema(
      "https://carelabz.com/de/about-us/",
      page?.metaTitle ?? "About Carelabs — Power System Consultants Germany",
      page?.metaDescription ?? "Carelabs is a leading electrical safety engineering firm in Germany.",
      "en-DE"
    ),
    getBreadcrumbSchema([
      { name: "Home", url: "https://carelabz.com/de/" },
      { name: "About", url: "https://carelabz.com/de/about-us/" },
    ]),
  ]);
        ...
        <section className="bg-[#0050B3] py-20 px-4">
          <div className="mx-auto max-w-4xl text-center">
            <h2 className="text-3xl md:text-4xl font-bold text-white mb-4">
              {page?.ctaBannerHeading ?? "Partner with Carelabs Germany"}
            </h2>
            {page?.ctaBannerSubtext ? (
              <p className="text-white/70 text-lg mb-8">{page.ctaBannerSubtext}</p>
            ) : (
              <p className="text-white/70 text-lg mb-8">
                Ready to elevate your safety standards? Talk to our Germany experts today.
              </p>
            )}
            <Link
              href={page?.ctaBannerPrimaryHref ?? "/de/contact-us/"}
