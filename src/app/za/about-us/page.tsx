...
export async function generateMetadata(): Promise<Metadata> {
  const page = await getAboutPage("za");
  return {
    title: page?.metaTitle ?? "About Carelabs | South African Electrical Safety Experts",
    description:
      page?.metaDescription ??
      "Learn about Carelabs — our mission, values, and the team dedicated to electrical safety testing and SANS 10142 compliance across South Africa.",
    alternates: {
      canonical: "https://carelabz.com/za/about-us/",
      languages: {
        "en-ZA": "https://carelabz.com/za/about-us/",
        "x-default": "https://carelabz.com/za/about-us/",
      },
    },
    openGraph: {
      title: page?.metaTitle ?? "About Carelabs — Power System Consultants South Africa",
      description:
        page?.metaDescription ?? "Carelabs is a leading electrical safety engineering firm in South Africa.",
      url: "https://carelabz.com/za/about-us/",
      siteName: "Carelabs",
      type: "website",
    },
    twitter: {
...
export default async function ZAAboutPage() {
  const page = await getAboutPage("za");

  const headline = page?.heroHeadline ?? "Who We Are";
  const subtext =
    page?.heroSubtext ??
    "Carelabs is a trusted partner for electrical safety testing, calibration, inspection, and certification services across South Africa.";
  ...
  const jsonLd = buildJsonLd([
    ...
    getWebPageSchema(
      "https://carelabz.com/za/about-us/",
      page?.metaTitle ?? "About Carelabs — Power System Consultants South Africa",
      page?.metaDescription ?? "Carelabs is a leading electrical safety engineering firm in South Africa.",
      "en-ZA"
    ),
    getBreadcrumbSchema([
      { name: "Home", url: "https://carelabz.com/za/" },
      { name: "About", url: "https://carelabz.com/za/about-us/" },
    ]),
  ]);
        ...
        <section className="bg-[#0050B3] py-20 px-4">
          <div className="mx-auto max-w-4xl text-center">
            <h2 className="text-3xl md:text-4xl font-bold text-white mb-4">
              {page?.ctaBannerHeading ?? "Partner with Carelabs South Africa"}
            </h2>
            {page?.ctaBannerSubtext ? (
              <p className="text-white/70 text-lg mb-8">{page.ctaBannerSubtext}</p>
            ) : (
              <p className="text-white/70 text-lg mb-8">
                Ready to elevate your safety standards? Talk to our South African experts today.
              </p>
            )}
            <Link
              href={page?.ctaBannerPrimaryHref ?? "/za/contact-us/"}
