...
export async function generateMetadata(): Promise<Metadata> {
  const page = await getContactPage("cn");
  return {
    title: page?.metaTitle ?? "Contact Carelabs China | Get in Touch",
    description:
      page?.metaDescription ??
      "Contact the Carelabs team for electrical safety testing, arc flash studies, and GB 50054 compliance services across China.",
    alternates: {
      canonical: "https://carelabz.com/cn/contact-us/",
      languages: {
        "en-CN": "https://carelabz.com/cn/contact-us/",
        "x-default": "https://carelabz.com/cn/contact-us/",
      },
    },
    openGraph: {
      title: page?.metaTitle ?? "Contact Carelabs — Get a Free Consultation China",
      description:
        page?.metaDescription ?? "Get in touch for a free consultation on electrical safety services in China.",
      url: "https://carelabz.com/cn/contact-us/",
      siteName: "Carelabs",
      type: "website",
    },
    twitter: {
...
export default async function CNContactPage() {
  const headersList = headers();
  const iso2 = getCountryFromHeaders(headersList, "CN");
  // On the China contact page the banner must always say China
  // regardless of the visitor's IP — the services ARE in China.
  const countryName = "China";

  const [page, services] = await Promise.all([
    getContactPage("cn"),
    getServicesByRegion("cn"),
  ]);
  ...
  const subtext =
    page?.heroSubtext ??
    "Have a question or ready to start a project? Our Chinan team is here to help.";
  ...
  const jsonLd = buildJsonLd([
    ...
    getWebPageSchema(
      "https://carelabz.com/cn/contact-us/",
      page?.metaTitle ?? "Contact Carelabs — Get a Free Consultation China",
      page?.metaDescription ?? "Get in touch for a free consultation on electrical safety services in China.",
      "en-CN"
    ),
    getBreadcrumbSchema([
      { name: "Home", url: "https://carelabz.com/cn/" },
      { name: "Contact", url: "https://carelabz.com/cn/contact-us/" },
    ]),
  ]);
                ...
                {page?.mapEmbedUrl && (
                  <div className="rounded-xl overflow-hidden border border-gray-200 mt-4">
                    <iframe
                      ...
                      title="Carelabs China Office Location"
                    />
                  </div>
                )}
              </div>
        ...
        <section className="bg-[#0050B3] py-20 px-4">
          <div className="mx-auto max-w-4xl text-center">
            ...
            <p className="text-white/70 text-lg mb-8">
              Our Chinan team is available to respond to emergency compliance and
