...
export async function generateMetadata(): Promise<Metadata> {
  const page = await getAboutPage("be");
  return {
    title: page?.metaTitle ?? "About Carelabs | Belgium Electrical Safety Experts",
    description:
      page?.metaDescription ??
      "Learn about Carelabs — our mission, values, and the team dedicated to electrical safety testing and AREI/RGIE compliance across Belgium.",
    alternates: {
      canonical: "https://carelabz.com/be/about-us/",
      languages: {
        "en-BE": "https://carelabz.com/be/about-us/",
        "x-default": "https://carelabz.com/be/about-us/",
      },
    },
    openGraph: {
      title: page?.metaTitle ?? "About Carelabs — Power System Consultants Belgium",
      description:
        page?.metaDescription ?? "Carelabs is a leading electrical safety engineering firm in Belgium.",
      url: "https://carelabz.com/be/about-us/",
      siteName: "Carelabs",
      type: "website",
    },
    twitter: {
...
export default async function BEAboutPage() {
  const page = await getAboutPage("be");

  const headline = page?.heroHeadline ?? "Who We Are";
  const subtext =
    page?.heroSubtext ??
    "Carelabs is a trusted partner for electrical safety testing, calibration, inspection, and certification services across Belgium.";
  ...
  const jsonLd = buildJsonLd([
    ...
    getWebPageSchema(
      "https://carelabz.com/be/about-us/",
      page?.metaTitle ?? "About Carelabs — Power System Consultants Belgium",
      page?.metaDescription ?? "Carelabs is a leading electrical safety engineering firm in Belgium.",
      "en-BE"
    ),
    getBreadcrumbSchema([
      { name: "Home", url: "https://carelabz.com/be/" },
      { name: "About", url: "https://carelabz.com/be/about-us/" },
    ]),
  ]);
        ...
        <section className="bg-[#0050B3] py-20 px-4">
          <div className="mx-auto max-w-4xl text-center">
            <h2 className="text-3xl md:text-4xl font-bold text-white mb-4">
              {page?.ctaBannerHeading ?? "Partner with Carelabs Belgium"}
            </h2>
            {page?.ctaBannerSubtext ? (
              <p className="text-white/70 text-lg mb-8">{page.ctaBannerSubtext}</p>
            ) : (
              <p className="text-white/70 text-lg mb-8">
                Ready to elevate your safety standards? Talk to our Belgium experts today.
              </p>
            )}
            <Link
              href={page?.ctaBannerPrimaryHref ?? "/be/contact-us/"}
