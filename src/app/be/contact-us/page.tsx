...
const config = COUNTRY_CONFIGS["be"];
...
export async function generateMetadata(): Promise<Metadata> {
  const page = await getContactPage("be");
  return {
    title: page?.metaTitle ?? "Contact Carelabs Belgium | Get in Touch",
    description:
      page?.metaDescription ??
      "Contact the Carelabs team for electrical safety testing, arc flash studies, and AREI/RGIE compliance services across Belgium.",
    alternates: {
      canonical: "https://carelabz.com/be/contact-us/",
      languages: {
        "en-BE": "https://carelabz.com/be/contact-us/",
        "x-default": "https://carelabz.com/be/contact-us/",
      },
    },
    openGraph: {
      title: page?.metaTitle ?? "Contact Carelabs — Get a Free Consultation Belgium",
      description:
        page?.metaDescription ?? "Get in touch for a free consultation on electrical safety services in Belgium.",
      url: "https://carelabz.com/be/contact-us/",
      siteName: "Carelabs",
      type: "website",
    },
    twitter: {
...
export default async function BEContactPage() {
  const headersList = headers();
  const iso2 = getCountryFromHeaders(headersList, "BE");
  // On the Belgium contact page the banner must always say Belgium
  // regardless of the visitor's IP — the services ARE in Belgium.
  const countryName = "Belgium";

  const [page, services] = await Promise.all([
    getContactPage("be"),
    getServicesByRegion("be"),
  ]);
  ...
  const subtext =
    page?.heroSubtext ??
    "Have a question or ready to start a project? Our Belgium team is here to help.";

  const jsonLd = buildJsonLd([
    getRegionOrganizationSchema({ cc: "be", countryName: "Belgium", countryIso2: "BE", phone: config.phone, email: config.email, addressLocality: config.address }),
    getWebPageSchema(
      "https://carelabz.com/be/contact-us/",
      page?.metaTitle ?? "Contact Carelabs — Get a Free Consultation Belgium",
      page?.metaDescription ?? "Get in touch for a free consultation on electrical safety services in Belgium.",
      "en-BE"
    ),
    getBreadcrumbSchema([
      { name: "Home", url: "https://carelabz.com/be/" },
      { name: "Contact", url: "https://carelabz.com/be/contact-us/" },
    ]),
  ]);
                ...
                {page?.mapEmbedUrl && (
                  <div className="rounded-xl overflow-hidden border border-gray-200 mt-4">
                    <iframe
                      ...
                      title="Carelabs Belgium Office Location"
                    />
                  </div>
                )}
              </div>
        ...
        <section className="bg-[#0050B3] py-20 px-4">
          <div className="mx-auto max-w-4xl text-center">
            ...
            <p className="text-white/70 text-lg mb-8">
              Our Belgium team is available to respond to emergency compliance and
