...
const config = COUNTRY_CONFIGS["vn"];
...
export async function generateMetadata(): Promise<Metadata> {
  const page = await getContactPage("vn");
  return {
    title: page?.metaTitle ?? "Contact Carelabs Vietnam | Get in Touch",
    description:
      page?.metaDescription ??
      "Contact the Carelabs team for electrical safety testing, arc flash studies, and TCVN 7447 compliance services across Vietnam.",
    alternates: {
      canonical: "https://carelabz.com/vn/contact-us/",
      languages: {
        "en-VN": "https://carelabz.com/vn/contact-us/",
        "x-default": "https://carelabz.com/vn/contact-us/",
      },
    },
    openGraph: {
      title: page?.metaTitle ?? "Contact Carelabs — Get a Free Consultation Vietnam",
      description:
        page?.metaDescription ?? "Get in touch for a free consultation on electrical safety services in Vietnam.",
      url: "https://carelabz.com/vn/contact-us/",
      siteName: "Carelabs",
      type: "website",
    },
    twitter: {
...
export default async function VNContactPage() {
  const headersList = headers();
  const iso2 = getCountryFromHeaders(headersList, "VN");
  // On the Vietnam contact page the banner must always say Vietnam
  // regardless of the visitor's IP — the services ARE in Vietnam.
  const countryName = "Vietnam";

  const [page, services] = await Promise.all([
    getContactPage("vn"),
    getServicesByRegion("vn"),
  ]);
  ...
  const subtext =
    page?.heroSubtext ??
    "Have a question or ready to start a project? Our Vietnam team is here to help.";

  const jsonLd = buildJsonLd([
    getRegionOrganizationSchema({ cc: "vn", countryName: "Vietnam", countryIso2: "VN", phone: config.phone, email: config.email, addressLocality: config.address }),
    getWebPageSchema(
      "https://carelabz.com/vn/contact-us/",
      page?.metaTitle ?? "Contact Carelabs — Get a Free Consultation Vietnam",
      page?.metaDescription ?? "Get in touch for a free consultation on electrical safety services in Vietnam.",
      "en-VN"
    ),
    getBreadcrumbSchema([
      { name: "Home", url: "https://carelabz.com/vn/" },
      { name: "Contact", url: "https://carelabz.com/vn/contact-us/" },
    ]),
  ]);
                ...
                {page?.mapEmbedUrl && (
                  <div className="rounded-xl overflow-hidden border border-gray-200 mt-4">
                    <iframe
                      ...
                      title="Carelabs Vietnam Office Location"
                    />
                  </div>
                )}
              </div>
        ...
        <section className="bg-[#0050B3] py-20 px-4">
          <div className="mx-auto max-w-4xl text-center">
            ...
            <p className="text-white/70 text-lg mb-8">
              Our Vietnam team is available to respond to emergency compliance and
