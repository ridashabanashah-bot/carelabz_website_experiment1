...
export async function generateMetadata(): Promise<Metadata> {
  const page = await getAboutPage("vn");
  return {
    title: page?.metaTitle ?? "About Carelabs | Vietnam Electrical Safety Experts",
    description:
      page?.metaDescription ??
      "Learn about Carelabs — our mission, values, and the team dedicated to electrical safety testing and TCVN 7447 compliance across Vietnam.",
    alternates: {
      canonical: "https://carelabz.com/vn/about-us/",
      languages: {
        "en-VN": "https://carelabz.com/vn/about-us/",
        "x-default": "https://carelabz.com/vn/about-us/",
      },
    },
    openGraph: {
      title: page?.metaTitle ?? "About Carelabs — Power System Consultants Vietnam",
      description:
        page?.metaDescription ?? "Carelabs is a leading electrical safety engineering firm in Vietnam.",
      url: "https://carelabz.com/vn/about-us/",
      siteName: "Carelabs",
      type: "website",
    },
    twitter: {
...
export default async function VNAboutPage() {
  const page = await getAboutPage("vn");

  const headline = page?.heroHeadline ?? "Who We Are";
  const subtext =
    page?.heroSubtext ??
    "Carelabs is a trusted partner for electrical safety testing, calibration, inspection, and certification services across Vietnam.";
  ...
  const jsonLd = buildJsonLd([
    ...
    getWebPageSchema(
      "https://carelabz.com/vn/about-us/",
      page?.metaTitle ?? "About Carelabs — Power System Consultants Vietnam",
      page?.metaDescription ?? "Carelabs is a leading electrical safety engineering firm in Vietnam.",
      "en-VN"
    ),
    getBreadcrumbSchema([
      { name: "Home", url: "https://carelabz.com/vn/" },
      { name: "About", url: "https://carelabz.com/vn/about-us/" },
    ]),
  ]);
        ...
        <section className="bg-[#0050B3] py-20 px-4">
          <div className="mx-auto max-w-4xl text-center">
            <h2 className="text-3xl md:text-4xl font-bold text-white mb-4">
              {page?.ctaBannerHeading ?? "Partner with Carelabs Vietnam"}
            </h2>
            {page?.ctaBannerSubtext ? (
              <p className="text-white/70 text-lg mb-8">{page.ctaBannerSubtext}</p>
            ) : (
              <p className="text-white/70 text-lg mb-8">
                Ready to elevate your safety standards? Talk to our Vietnam experts today.
              </p>
            )}
            <Link
              href={page?.ctaBannerPrimaryHref ?? "/vn/contact-us/"}
