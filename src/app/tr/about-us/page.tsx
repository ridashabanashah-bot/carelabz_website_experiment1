...
export async function generateMetadata(): Promise<Metadata> {
  const page = await getAboutPage("tr");
  return {
    title: page?.metaTitle ?? "About Carelabs | Turkey Electrical Safety Experts",
    description:
      page?.metaDescription ??
      "Learn about Carelabs — our mission, values, and the team dedicated to electrical safety testing and TS HD 60364 compliance across Turkey.",
    alternates: {
      canonical: "https://carelabz.com/tr/about-us/",
      languages: {
        "en-TR": "https://carelabz.com/tr/about-us/",
        "x-default": "https://carelabz.com/tr/about-us/",
      },
    },
    openGraph: {
      title: page?.metaTitle ?? "About Carelabs — Power System Consultants Turkey",
      description:
        page?.metaDescription ?? "Carelabs is a leading electrical safety engineering firm in Turkey.",
      url: "https://carelabz.com/tr/about-us/",
      siteName: "Carelabs",
      type: "website",
    },
    twitter: {
...
export default async function TRAboutPage() {
  const page = await getAboutPage("tr");

  const headline = page?.heroHeadline ?? "Who We Are";
  const subtext =
    page?.heroSubtext ??
    "Carelabs is a trusted partner for electrical safety testing, calibration, inspection, and certification services across Turkey.";
  ...
  const jsonLd = buildJsonLd([
    ...
    getWebPageSchema(
      "https://carelabz.com/tr/about-us/",
      page?.metaTitle ?? "About Carelabs — Power System Consultants Turkey",
      page?.metaDescription ?? "Carelabs is a leading electrical safety engineering firm in Turkey.",
      "en-TR"
    ),
    getBreadcrumbSchema([
      { name: "Home", url: "https://carelabz.com/tr/" },
      { name: "About", url: "https://carelabz.com/tr/about-us/" },
    ]),
  ]);
        ...
        <section className="bg-[#0050B3] py-20 px-4">
          <div className="mx-auto max-w-4xl text-center">
            <h2 className="text-3xl md:text-4xl font-bold text-white mb-4">
              {page?.ctaBannerHeading ?? "Partner with Carelabs Turkey"}
            </h2>
            {page?.ctaBannerSubtext ? (
              <p className="text-white/70 text-lg mb-8">{page.ctaBannerSubtext}</p>
            ) : (
              <p className="text-white/70 text-lg mb-8">
                Ready to elevate your safety standards? Talk to our Turkey experts today.
              </p>
            )}
            <Link
              href={page?.ctaBannerPrimaryHref ?? "/tr/contact-us/"}
