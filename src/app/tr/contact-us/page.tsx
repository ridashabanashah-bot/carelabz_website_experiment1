...
const config = COUNTRY_CONFIGS["tr"];
...
export async function generateMetadata(): Promise<Metadata> {
  const page = await getContactPage("tr");
  return {
    title: page?.metaTitle ?? "Contact Carelabs Turkey | Get in Touch",
    description:
      page?.metaDescription ??
      "Contact the Carelabs team for electrical safety testing, arc flash studies, and TS HD 60364 compliance services across Turkey.",
    alternates: {
      canonical: "https://carelabz.com/tr/contact-us/",
      languages: {
        "en-TR": "https://carelabz.com/tr/contact-us/",
        "x-default": "https://carelabz.com/tr/contact-us/",
      },
    },
    openGraph: {
      title: page?.metaTitle ?? "Contact Carelabs — Get a Free Consultation Turkey",
      description:
        page?.metaDescription ?? "Get in touch for a free consultation on electrical safety services in Turkey.",
      url: "https://carelabz.com/tr/contact-us/",
      siteName: "Carelabs",
      type: "website",
    },
    twitter: {
...
export default async function TRContactPage() {
  ...
  const iso2 = getCountryFromHeaders(headersList, "TR");
  // On the Turkey contact page the banner must always say Turkey
  // regardless of the visitor's IP — the services ARE in Turkey.
  const countryName = "Turkey";

  const [page, services] = await Promise.all([
    getContactPage("tr"),
    getServicesByRegion("tr"),
  ]);
  ...
  const subtext =
    page?.heroSubtext ??
    "Have a question or ready to start a project? Our Turkey team is here to help.";

  const jsonLd = buildJsonLd([
    getRegionOrganizationSchema({ cc: "tr", countryName: "Turkey", countryIso2: "TR", phone: config.phone, email: config.email, addressLocality: config.address }),
    getWebPageSchema(
      "https://carelabz.com/tr/contact-us/",
      page?.metaTitle ?? "Contact Carelabs — Get a Free Consultation Turkey",
      page?.metaDescription ?? "Get in touch for a free consultation on electrical safety services in Turkey.",
      "en-TR"
    ),
    getBreadcrumbSchema([
      { name: "Home", url: "https://carelabz.com/tr/" },
      { name: "Contact", url: "https://carelabz.com/tr/contact-us/" },
    ]),
  ]);
                ...
                {page?.mapEmbedUrl && (
                  <div className="rounded-xl overflow-hidden border border-gray-200 mt-4">
                    <iframe
                      ...
                      title="Carelabs Turkey Office Location"
                    />
                  </div>
                )}
              </div>
        ...
        <section className="bg-[#0050B3] py-20 px-4">
          <div className="mx-auto max-w-4xl text-center">
            ...
            <p className="text-white/70 text-lg mb-8">
              Our Turkey team is available to respond to emergency compliance and
