...
const config = COUNTRY_CONFIGS["ro"];
...
export async function generateMetadata(): Promise<Metadata> {
  const page = await getContactPage("ro");
  return {
    title: page?.metaTitle ?? "Contact Carelabs Romania | Get in Touch",
    description:
      page?.metaDescription ??
      "Contact the Carelabs team for electrical safety testing, arc flash studies, and I7-2011 compliance services across Romania.",
    alternates: {
      canonical: "https://carelabz.com/ro/contact-us/",
      languages: {
        "en-RO": "https://carelabz.com/ro/contact-us/",
        "x-default": "https://carelabz.com/ro/contact-us/",
      },
    },
    openGraph: {
      title: page?.metaTitle ?? "Contact Carelabs — Get a Free Consultation Romania",
      description:
        page?.metaDescription ?? "Get in touch for a free consultation on electrical safety services in Romania.",
      url: "https://carelabz.com/ro/contact-us/",
      siteName: "Carelabs",
      type: "website",
    },
    twitter: {
...
export default async function ROContactPage() {
  const headersList = headers();
  const iso2 = getCountryFromHeaders(headersList, "RO");
  // On the Romania contact page the banner must always say Romania
  // regardless of the visitor's IP — the services ARE in Romania.
  const countryName = "Romania";

  const [page, services] = await Promise.all([
    getContactPage("ro"),
    getServicesByRegion("ro"),
  ]);
  ...
  const subtext =
    page?.heroSubtext ??
    "Have a question or ready to start a project? Our Romanian team is here to help.";

  const jsonLd = buildJsonLd([
    getRegionOrganizationSchema({ cc: "ro", countryName: "Romania", countryIso2: "RO", phone: config.phone, email: config.email, addressLocality: config.address }),
    getWebPageSchema(
      "https://carelabz.com/ro/contact-us/",
      page?.metaTitle ?? "Contact Carelabs — Get a Free Consultation Romania",
      page?.metaDescription ?? "Get in touch for a free consultation on electrical safety services in Romania.",
      "en-RO"
    ),
    getBreadcrumbSchema([
      { name: "Home", url: "https://carelabz.com/ro/" },
      { name: "Contact", url: "https://carelabz.com/ro/contact-us/" },
    ]),
  ]);
                ...
                {page?.mapEmbedUrl && (
                  <div className="rounded-xl overflow-hidden border border-gray-200 mt-4">
                    <iframe
                      ...
                      title="Carelabs Romania Office Location"
                    />
                  </div>
                )}
              </div>
        ...
        <section className="bg-[#0050B3] py-20 px-4">
          <div className="mx-auto max-w-4xl text-center">
            ...
            <p className="text-white/70 text-lg mb-8">
              Our Romanian team is available to respond to emergency compliance and
