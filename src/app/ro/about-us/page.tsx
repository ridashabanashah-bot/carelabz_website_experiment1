...
export async function generateMetadata(): Promise<Metadata> {
  const page = await getAboutPage("ro");
  return {
    title: page?.metaTitle ?? "About Carelabs | Romanian Electrical Safety Experts",
    description:
      page?.metaDescription ??
      "Learn about Carelabs — our mission, values, and the team dedicated to electrical safety testing and I7-2011 compliance across Romania.",
    alternates: {
      canonical: "https://carelabz.com/ro/about-us/",
      languages: {
        "en-RO": "https://carelabz.com/ro/about-us/",
        "x-default": "https://carelabz.com/ro/about-us/",
      },
    },
    openGraph: {
      title: page?.metaTitle ?? "About Carelabs — Power System Consultants Romania",
      description:
        page?.metaDescription ?? "Carelabs is a leading electrical safety engineering firm in Romania.",
      url: "https://carelabz.com/ro/about-us/",
      siteName: "Carelabs",
      type: "website",
    },
    twitter: {
...
export default async function ROAboutPage() {
  const page = await getAboutPage("ro");

  const headline = page?.heroHeadline ?? "Who We Are";
  const subtext =
    page?.heroSubtext ??
    "Carelabs is a trusted partner for electrical safety testing, calibration, inspection, and certification services across Romania.";
  ...
  const jsonLd = buildJsonLd([
    ...
    getWebPageSchema(
      "https://carelabz.com/ro/about-us/",
      page?.metaTitle ?? "About Carelabs — Power System Consultants Romania",
      page?.metaDescription ?? "Carelabs is a leading electrical safety engineering firm in Romania.",
      "en-RO"
    ),
    getBreadcrumbSchema([
      { name: "Home", url: "https://carelabz.com/ro/" },
      { name: "About", url: "https://carelabz.com/ro/about-us/" },
    ]),
  ]);
        ...
        <section className="bg-[#0050B3] py-20 px-4">
          <div className="mx-auto max-w-4xl text-center">
            <h2 className="text-3xl md:text-4xl font-bold text-white mb-4">
              {page?.ctaBannerHeading ?? "Partner with Carelabs Romania"}
            </h2>
            {page?.ctaBannerSubtext ? (
              <p className="text-white/70 text-lg mb-8">{page.ctaBannerSubtext}</p>
            ) : (
              <p className="text-white/70 text-lg mb-8">
                Ready to elevate your safety standards? Talk to our Romanian experts today.
              </p>
            )}
            <Link
              href={page?.ctaBannerPrimaryHref ?? "/ro/contact-us/"}
