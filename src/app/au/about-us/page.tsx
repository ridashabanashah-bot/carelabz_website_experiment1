...
export async function generateMetadata(): Promise<Metadata> {
  const page = await getAboutPage("au");
  return {
    title: page?.metaTitle ?? "About Carelabs | Australian Electrical Safety Experts",
    description:
      page?.metaDescription ??
      "Learn about Carelabs — our mission, values, and the team dedicated to electrical safety testing and AS/NZS 3000 compliance across Australia.",
    alternates: {
      canonical: "https://carelabz.com/au/about-us/",
      languages: {
        "en-AU": "https://carelabz.com/au/about-us/",
        "x-default": "https://carelabz.com/au/about-us/",
      },
    },
    openGraph: {
      title: page?.metaTitle ?? "About Carelabs — Power System Consultants Australia",
      description:
        page?.metaDescription ?? "Carelabs is a leading electrical safety engineering firm in Australia.",
      url: "https://carelabz.com/au/about-us/",
      siteName: "Carelabs",
      type: "website",
    },
    twitter: {
...
export default async function AUAboutPage() {
  const page = await getAboutPage("au");

  const headline = page?.heroHeadline ?? "Who We Are";
  const subtext =
    page?.heroSubtext ??
    "Carelabs is a trusted partner for electrical safety testing, calibration, inspection, and certification services across Australia.";
  ...
  const jsonLd = buildJsonLd([
    ...
    getWebPageSchema(
      "https://carelabz.com/au/about-us/",
      page?.metaTitle ?? "About Carelabs — Power System Consultants Australia",
      page?.metaDescription ?? "Carelabs is a leading electrical safety engineering firm in Australia.",
      "en-AU"
    ),
    getBreadcrumbSchema([
      { name: "Home", url: "https://carelabz.com/au/" },
      { name: "About", url: "https://carelabz.com/au/about-us/" },
    ]),
  ]);
        ...
        <section className="bg-[#0050B3] py-20 px-4">
          <div className="mx-auto max-w-4xl text-center">
            <h2 className="text-3xl md:text-4xl font-bold text-white mb-4">
              {page?.ctaBannerHeading ?? "Partner with Carelabs Australia"}
            </h2>
            {page?.ctaBannerSubtext ? (
              <p className="text-white/70 text-lg mb-8">{page.ctaBannerSubtext}</p>
            ) : (
              <p className="text-white/70 text-lg mb-8">
                Ready to elevate your safety standards? Talk to our Australian experts today.
              </p>
            )}
            <Link
              href={page?.ctaBannerPrimaryHref ?? "/au/contact-us/"}
