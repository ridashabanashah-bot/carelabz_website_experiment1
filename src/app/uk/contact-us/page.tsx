...
const config = COUNTRY_CONFIGS["uk"];
...
export async function generateMetadata(): Promise<Metadata> {
  const page = await getContactPage("uk");
  return {
    title: page?.metaTitle ?? "Contact Carelabs United Kingdom | Get in Touch",
    description:
      page?.metaDescription ??
      "Contact the Carelabs team for electrical safety testing, arc flash studies, and BS 7671 compliance services across United Kingdom.",
    alternates: {
      canonical: "https://carelabz.com/uk/contact-us/",
      languages: {
        "en-GB": "https://carelabz.com/uk/contact-us/",
        "x-default": "https://carelabz.com/uk/contact-us/",
      },
    },
    openGraph: {
      title: page?.metaTitle ?? "Contact Carelabs — Get a Free Consultation United Kingdom",
      description:
        page?.metaDescription ?? "Get in touch for a free consultation on electrical safety services in United Kingdom.",
      url: "https://carelabz.com/uk/contact-us/",
      siteName: "Carelabs",
      type: "website",
    },
    twitter: {
...
export default async function UKContactPage() {
  ...
  const iso2 = getCountryFromHeaders(headersList, "UK");
  // On the United Kingdom contact page the banner must always say United Kingdom
  // regardless of the visitor's IP — the services ARE in United Kingdom.
  const countryName = "United Kingdom";

  const [page, services] = await Promise.all([
    getContactPage("uk"),
    getServicesByRegion("uk"),
  ]);
  ...
  const subtext =
    page?.heroSubtext ??
    "Have a question or ready to start a project? Our United Kingdom team is here to help.";

  const jsonLd = buildJsonLd([
    getRegionOrganizationSchema({ cc: "uk", countryName: "United Kingdom", countryIso2: "UK", phone: config.phone, email: config.email, addressLocality: config.address }),
    getWebPageSchema(
      "https://carelabz.com/uk/contact-us/",
      page?.metaTitle ?? "Contact Carelabs — Get a Free Consultation United Kingdom",
      page?.metaDescription ?? "Get in touch for a free consultation on electrical safety services in United Kingdom.",
      "en-GB"
    ),
    getBreadcrumbSchema([
      { name: "Home", url: "https://carelabz.com/uk/" },
      { name: "Contact", url: "https://carelabz.com/uk/contact-us/" },
    ]),
  ]);
                ...
                {page?.mapEmbedUrl && (
                  <div className="rounded-xl overflow-hidden border border-gray-200 mt-4">
                    <iframe
                      ...
                      title="Carelabs United Kingdom Office Location"
                    />
                  </div>
                )}
              </div>
        ...
        <section className="bg-[#0050B3] py-20 px-4">
          <div className="mx-auto max-w-4xl text-center">
            ...
            <p className="text-white/70 text-lg mb-8">
              Our United Kingdom team is available to respond to emergency compliance and
