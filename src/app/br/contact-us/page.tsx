...
export async function generateMetadata(): Promise<Metadata> {
  const page = await getContactPage("br");
  return {
    title: page?.metaTitle ?? "Contact Carelabs Brazil | Get in Touch",
    description:
      page?.metaDescription ??
      "Contact the Carelabs team for electrical safety testing, arc flash studies, and NR-10 compliance services across Brazil.",
    alternates: {
      canonical: "https://carelabz.com/br/contact-us/",
      languages: {
        "en-BR": "https://carelabz.com/br/contact-us/",
        "x-default": "https://carelabz.com/br/contact-us/",
      },
    },
    openGraph: {
      title: page?.metaTitle ?? "Contact Carelabs — Get a Free Consultation Brazil",
      description:
        page?.metaDescription ?? "Get in touch for a free consultation on electrical safety services in Brazil.",
      url: "https://carelabz.com/br/contact-us/",
      siteName: "Carelabs",
      type: "website",
    },
    twitter: {
...
export default async function BRContactPage() {
  ...
  const iso2 = getCountryFromHeaders(headersList, "BR");
  // On the Brazil contact page the banner must always say Brazil
  // regardless of the visitor's IP — the services ARE in Brazil.
  const countryName = "Brazil";

  const [page, services] = await Promise.all([
    getContactPage("br"),
    getServicesByRegion("br"),
  ]);
  ...
  const subtext =
    page?.heroSubtext ??
    "Have a question or ready to start a project? Our Brazil team is here to help.";
  ...
  const jsonLd = buildJsonLd([
    ...
    getWebPageSchema(
      "https://carelabz.com/br/contact-us/",
      page?.metaTitle ?? "Contact Carelabs — Get a Free Consultation Brazil",
      page?.metaDescription ?? "Get in touch for a free consultation on electrical safety services in Brazil.",
      "en-BR"
    ),
    getBreadcrumbSchema([
      { name: "Home", url: "https://carelabz.com/br/" },
      { name: "Contact", url: "https://carelabz.com/br/contact-us/" },
    ]),
  ]);
                ...
                {page?.mapEmbedUrl && (
                  <div className="rounded-xl overflow-hidden border border-gray-200 mt-4">
                    <iframe
                      ...
                      title="Carelabs Brazil Office Location"
                    />
                  </div>
                )}
              </div>
        ...
        <section className="bg-[#0050B3] py-20 px-4">
          <div className="mx-auto max-w-4xl text-center">
            ...
            <p className="text-white/70 text-lg mb-8">
              Our Brazil team is available to respond to emergency compliance and
