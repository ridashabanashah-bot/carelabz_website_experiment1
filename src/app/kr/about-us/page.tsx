...
export async function generateMetadata(): Promise<Metadata> {
  const page = await getAboutPage("kr");
  return {
    title: page?.metaTitle ?? "About Carelabs | South Korean Electrical Safety Experts",
    description:
      page?.metaDescription ??
      "Learn about Carelabs — our mission, values, and the team dedicated to electrical safety testing and KEC compliance across South Korea.",
    alternates: {
      canonical: "https://carelabz.com/kr/about-us/",
      languages: {
        "en-KR": "https://carelabz.com/kr/about-us/",
        "x-default": "https://carelabz.com/kr/about-us/",
      },
    },
    openGraph: {
      title: page?.metaTitle ?? "About Carelabs — Power System Consultants South Korea",
      description:
        page?.metaDescription ?? "Carelabs is a leading electrical safety engineering firm in South Korea.",
      url: "https://carelabz.com/kr/about-us/",
      siteName: "Carelabs",
      type: "website",
    },
    twitter: {
...
export default async function KRAboutPage() {
  const page = await getAboutPage("kr");

  const headline = page?.heroHeadline ?? "Who We Are";
  const subtext =
    page?.heroSubtext ??
    "Carelabs is a trusted partner for electrical safety testing, calibration, inspection, and certification services across South Korea.";
  ...
  const jsonLd = buildJsonLd([
    ...
    getWebPageSchema(
      "https://carelabz.com/kr/about-us/",
      page?.metaTitle ?? "About Carelabs — Power System Consultants South Korea",
      page?.metaDescription ?? "Carelabs is a leading electrical safety engineering firm in South Korea.",
      "en-KR"
    ),
    getBreadcrumbSchema([
      { name: "Home", url: "https://carelabz.com/kr/" },
      { name: "About", url: "https://carelabz.com/kr/about-us/" },
    ]),
  ]);
        ...
        <section className="bg-[#0050B3] py-20 px-4">
          <div className="mx-auto max-w-4xl text-center">
            <h2 className="text-3xl md:text-4xl font-bold text-white mb-4">
              {page?.ctaBannerHeading ?? "Partner with Carelabs South Korea"}
            </h2>
            {page?.ctaBannerSubtext ? (
              <p className="text-white/70 text-lg mb-8">{page.ctaBannerSubtext}</p>
            ) : (
              <p className="text-white/70 text-lg mb-8">
                Ready to elevate your safety standards? Talk to our South Korean experts today.
              </p>
            )}
            <Link
              href={page?.ctaBannerPrimaryHref ?? "/kr/contact-us/"}
