...
export async function generateMetadata(): Promise<Metadata> {
  const page = await getContactPage("kr");
  return {
    title: page?.metaTitle ?? "Contact Carelabs South Korea | Get in Touch",
    description:
      page?.metaDescription ??
      "Contact the Carelabs team for electrical safety testing, arc flash studies, and KEC compliance services across South Korea.",
    alternates: {
      canonical: "https://carelabz.com/kr/contact-us/",
      languages: {
        "en-KR": "https://carelabz.com/kr/contact-us/",
        "x-default": "https://carelabz.com/kr/contact-us/",
      },
    },
    openGraph: {
      title: page?.metaTitle ?? "Contact Carelabs — Get a Free Consultation South Korea",
      description:
        page?.metaDescription ?? "Get in touch for a free consultation on electrical safety services in South Korea.",
      url: "https://carelabz.com/kr/contact-us/",
      siteName: "Carelabs",
      type: "website",
    },
    twitter: {
...
export default async function KRContactPage() {
  const headersList = headers();
  const iso2 = getCountryFromHeaders(headersList, "KR");
  // On the South Korea contact page the banner must always say South Korea
  // regardless of the visitor's IP — the services ARE in South Korea.
  const countryName = "South Korea";

  const [page, services] = await Promise.all([
    getContactPage("kr"),
    getServicesByRegion("kr"),
  ]);
  ...
  const subtext =
    page?.heroSubtext ??
    "Have a question or ready to start a project? Our South Korean team is here to help.";
  ...
  const jsonLd = buildJsonLd([
    ...
    getWebPageSchema(
      "https://carelabz.com/kr/contact-us/",
      page?.metaTitle ?? "Contact Carelabs — Get a Free Consultation South Korea",
      page?.metaDescription ?? "Get in touch for a free consultation on electrical safety services in South Korea.",
      "en-KR"
    ),
    getBreadcrumbSchema([
      { name: "Home", url: "https://carelabz.com/kr/" },
      { name: "Contact", url: "https://carelabz.com/kr/contact-us/" },
    ]),
  ]);
                ...
                {page?.mapEmbedUrl && (
                  <div className="rounded-xl overflow-hidden border border-gray-200 mt-4">
                    <iframe
                      ...
                      title="Carelabs South Korea Office Location"
                    />
                  </div>
                )}
              </div>
        ...
        <section className="bg-[#0050B3] py-20 px-4">
          <div className="mx-auto max-w-4xl text-center">
            ...
            <p className="text-white/70 text-lg mb-8">
              Our South Korean team is available to respond to emergency compliance and
