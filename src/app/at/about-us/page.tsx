...
export async function generateMetadata(): Promise<Metadata> {
  const page = await getAboutPage("at");
  return {
    title: page?.metaTitle ?? "About Carelabs | Austrian Electrical Safety Experts",
    description:
      page?.metaDescription ??
      "Learn about Carelabs — our mission, values, and the team dedicated to electrical safety testing and ÖVE/ÖNORM E 8001 compliance across Austria.",
    alternates: {
      canonical: "https://carelabz.com/at/about-us/",
      languages: {
        "en-AT": "https://carelabz.com/at/about-us/",
        "x-default": "https://carelabz.com/at/about-us/",
      },
    },
    openGraph: {
      title: page?.metaTitle ?? "About Carelabs — Power System Consultants Austria",
      description:
        page?.metaDescription ?? "Carelabs is a leading electrical safety engineering firm in Austria.",
      url: "https://carelabz.com/at/about-us/",
      siteName: "Carelabs",
      type: "website",
    },
    twitter: {
...
export default async function ATAboutPage() {
  const page = await getAboutPage("at");

  const headline = page?.heroHeadline ?? "Who We Are";
  const subtext =
    page?.heroSubtext ??
    "Carelabs is a trusted partner for electrical safety testing, calibration, inspection, and certification services across Austria.";
  ...
  const jsonLd = buildJsonLd([
    ...
    getWebPageSchema(
      "https://carelabz.com/at/about-us/",
      page?.metaTitle ?? "About Carelabs — Power System Consultants Austria",
      page?.metaDescription ?? "Carelabs is a leading electrical safety engineering firm in Austria.",
      "en-AT"
    ),
    getBreadcrumbSchema([
      { name: "Home", url: "https://carelabz.com/at/" },
      { name: "About", url: "https://carelabz.com/at/about-us/" },
    ]),
  ]);
        ...
        <section className="bg-[#0050B3] py-20 px-4">
          <div className="mx-auto max-w-4xl text-center">
            <h2 className="text-3xl md:text-4xl font-bold text-white mb-4">
              {page?.ctaBannerHeading ?? "Partner with Carelabs Austria"}
            </h2>
            {page?.ctaBannerSubtext ? (
              <p className="text-white/70 text-lg mb-8">{page.ctaBannerSubtext}</p>
            ) : (
              <p className="text-white/70 text-lg mb-8">
                Ready to elevate your safety standards? Talk to our Austrian experts today.
              </p>
            )}
            <Link
              href={page?.ctaBannerPrimaryHref ?? "/at/contact-us/"}
