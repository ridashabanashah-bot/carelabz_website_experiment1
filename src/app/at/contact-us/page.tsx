...
export async function generateMetadata(): Promise<Metadata> {
  const page = await getContactPage("at");
  return {
    title: page?.metaTitle ?? "Contact Carelabs Austria | Get in Touch",
    description:
      page?.metaDescription ??
      "Contact the Carelabs team for electrical safety testing, arc flash studies, and ÖVE/ÖNORM E 8001 compliance services across Austria.",
    alternates: {
      canonical: "https://carelabz.com/at/contact-us/",
      languages: {
        "en-AT": "https://carelabz.com/at/contact-us/",
        "x-default": "https://carelabz.com/at/contact-us/",
      },
    },
    openGraph: {
      title: page?.metaTitle ?? "Contact Carelabs — Get a Free Consultation Austria",
      description:
        page?.metaDescription ?? "Get in touch for a free consultation on electrical safety services in Austria.",
      url: "https://carelabz.com/at/contact-us/",
      siteName: "Carelabs",
      type: "website",
    },
    twitter: {
...
export default async function ATContactPage() {
  const headersList = headers();
  const iso2 = getCountryFromHeaders(headersList, "AT");
  // On the Austria contact page the banner must always say Austria
  // regardless of the visitor's IP — the services ARE in Austria.
  const countryName = "Austria";

  const [page, services] = await Promise.all([
    getContactPage("at"),
    getServicesByRegion("at"),
  ]);
  ...
  const subtext =
    page?.heroSubtext ??
    "Have a question or ready to start a project? Our Austrian team is here to help.";
  ...
  const jsonLd = buildJsonLd([
    ...
    getWebPageSchema(
      "https://carelabz.com/at/contact-us/",
      page?.metaTitle ?? "Contact Carelabs — Get a Free Consultation Austria",
      page?.metaDescription ?? "Get in touch for a free consultation on electrical safety services in Austria.",
      "en-AT"
    ),
    getBreadcrumbSchema([
      { name: "Home", url: "https://carelabz.com/at/" },
      { name: "Contact", url: "https://carelabz.com/at/contact-us/" },
    ]),
  ]);
                ...
                {page?.mapEmbedUrl && (
                  <div className="rounded-xl overflow-hidden border border-gray-200 mt-4">
                    <iframe
                      ...
                      title="Carelabs Austria Office Location"
                    />
                  </div>
                )}
              </div>
        ...
        <section className="bg-[#0050B3] py-20 px-4">
          <div className="mx-auto max-w-4xl text-center">
            ...
            <p className="text-white/70 text-lg mb-8">
              Our Austrian team is available to respond to emergency compliance and
