...
interface RegionMobileNavProps {
  config: CountryConfig;
}

export function RegionMobileNav({ config }: RegionMobileNavProps) {
      ...
      {isOpen && (
        <div className="absolute top-[74px] left-0 right-0 bg-white shadow-lg border-t border-slate-100 z-50 max-h-[calc(100vh-74px)] overflow-y-auto">
          <nav className="flex flex-col p-4">
            <Link href={config.aboutPath} onClick={close} className={linkClass}>
              About Us
            </Link>
            ...
            <Link href={config.contactPath} onClick={close} className={linkClass}>
              Contact Us
            </Link>
